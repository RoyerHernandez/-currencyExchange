...
namespace currencyExchange
{
    public partial class MainPage : ContentPage
    {
        ...
        private async void ConvertButton_Clicked(object sender, EventArgs e)
        {
            ...
            {
                await DisplayAlert("Advertencia", "Debes ingresar un valor", "Aceptar");
                return;
            }
            if (sourcePicker.SelectedIndex == -1)
            {
                await DisplayAlert("Advertencia", "Debes seleccionar la moneda origen", "Aceptar");
                return;
            }
            if (targetPicker.SelectedIndex == -1)
            {
                await DisplayAlert("Advertencia", "Debes seleccionar la moneda destio", "Aceptar");
                return;
            }
            ...
            var amountConvert = amount / Convert.ToDecimal(sourceRate) * Convert.ToDecimal(targetRate);

            convertLabel.Text = string.Format("{0:N2} {1} = {2:N2} {3}",
                amount,
        ...
        private async void LoadRates()
        {
            ...
            try
            {
                ...
                var url = "/api/latest.json?app_id=83ce2ca1f7204704aa348493fb3dc0b0";
                var response = await client.GetAsync(url);

                if (!response.IsSuccessStatusCode)
                {
                    await DisplayAlert("Error", response.IsSuccessStatusCode.ToString(), "Aceptar");
        ...
        private void LoadPicker(Picker picker)
        {
            ...
            picker.Items.Add("ARS - Pesos Argentinos");
            ...
            picker.Items.Add("EUR - Euros");
            picker.Items.Add("FJD");
            picker.Items.Add("FKP");
            picker.Items.Add("GBP - Libra esterlina");
            ...
            picker.Items.Add("JPY - Yen");
            picker.Items.Add("KES");
            picker.Items.Add("KGS");
            picker.Items.Add("KHR");
            picker.Items.Add("KMF");
            picker.Items.Add("KPW - Won Norcoreano");
            picker.Items.Add("KRW - Won Surcoreano");
            ...
            picker.Items.Add("MXN - Pesos Mexicanos");
